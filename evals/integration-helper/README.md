...
node scripts/run.mjs --runner opencode --opencode-model google-vertex/gemini-2.5-pro --case react-vite-email-password --skip-llm-judge
...
- `--opencode-command <command>`: override the OpenCode command. Defaults to `opencode` or `OPENCODE_COMMAND`.
- `--opencode-model <provider/model>`: select an OpenCode model. Defaults to `OPENCODE_MODEL`.
...
- OpenCode is an external CLI dependency. Install it with `npm install -g opencode-ai`, `brew install anomalyco/tap/opencode`, or set `OPENCODE_COMMAND="npx -y opencode-ai"` for ad hoc runs.
- Authenticate providers with `opencode auth login` or provider-specific environment variables. For Gemini through Google Vertex AI, set Google Cloud credentials such as `GOOGLE_CLOUD_PROJECT` and `GOOGLE_APPLICATION_CREDENTIALS`, then use `opencode models --refresh` to confirm the exact `provider/model` id for your installed OpenCode version.
