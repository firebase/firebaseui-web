...
async function generateLogoComponents(): Promise<void> {
  try {
    console.log("🎨 Generating Angular logo components from core SVG files...");
    ...
    console.log(`📁 Found ${brandDirectories.length} brand directories`);
    ...
    for (const brandDir of brandDirectories) {
      ...
      try {
        ...
        console.warn(`⚠️  Skipping ${brandName}: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
    }
    ...
    console.error("❌ Error generating logo components:", error);
    process.exit(1);
  }
}
