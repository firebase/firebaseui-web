...
import { YahooLogo, YahooSignInButton } from "./yahoo-sign-in-button";
...
describe("<TwitterSignInButton />", () => {
  ...
  it("renders with the correct provider", () => {
    ...
    expect(button.getAttribute("data-provider")).toBe("yahoo.com");
  });
  ...
  it("renders with the correct translated text", () => {
    ...
    expect(screen.getByText("Sign in with Yahoo")).toBeDefined();
  });
  ...
  it("renders with different translated text for different locales", () => {
    ...
    expect(screen.getByText("Iniciar sesión con Yahoo")).toBeDefined();
  });
...
describe("<TwitterLogo />", () => {
  ...
  it("forwards custom SVG props", () => {
    const { container } = render(<YahooLogo data-testid="custom-svg" className="foo" width={32} />);
