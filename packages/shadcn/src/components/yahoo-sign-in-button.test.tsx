...
import { YahooSignInButton } from "./yahoo-sign-in-button";
...
vi.mock("@firebase-oss/ui-react", async (importOriginal) => {
  ...
  return {
    ...mod,
    YahooLogo: ({ className, ...props }: any) => (
      <svg data-testid="yahoo-logo" className={className} {...props}>
...
describe("<YahooSignInButton />", () => {
  ...
  it("renders with default Yahoo provider", () => {
    ...
    expect(screen.getByTestId("provider-id")).toHaveTextContent("yahoo.com");
    expect(screen.getByTestId("yahoo-logo")).toBeInTheDocument();
    expect(screen.getByText("Sign in with Yahoo")).toBeInTheDocument();
  });

  it("renders with custom Yahoo provider", () => {
    const customProvider = new OAuthProvider("yahoo.com");
    ...
    render(
      <FirebaseUIProvider ui={ui}>
        <YahooSignInButton provider={customProvider} />
      </FirebaseUIProvider>
    );
  ...
  it("renders Yahoo logo with correct props", () => {
    const ui = createMockUI({
      locale: registerLocale("test", {
        labels: {
          signInWithYahoo: "Sign in with Yahoo",
    ...
    const yahooLogo = screen.getByTestId("yahoo-logo");
  ...
  it("uses correct translation for button text", () => {
    const ui = createMockUI({
      locale: registerLocale("test", {
        labels: {
          signInWithYahoo: "Custom Yahoo Sign In Text",
        },
      }),
    });
    ...
    expect(screen.getByText("Custom Yahoo Sign In Text")).toBeInTheDocument();
  });
  ...
  it("renders children correctly", () => {
    ...
    // Should contain both the Yahoo logo and the text
    expect(childrenContainer.querySelector('[data-testid="yahoo-logo"]')).toBeInTheDocument();
    expect(childrenContainer).toHaveTextContent("Sign in with Yahoo");
  });
  ...
  it("passes onSignIn prop to OAuthButton", () => {
    ...
    render(
      <FirebaseUIProvider ui={ui}>
        <YahooSignInButton onSignIn={onSignIn} />
      </FirebaseUIProvider>
    );
