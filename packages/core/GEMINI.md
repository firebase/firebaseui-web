...
This document provides context for the `@invertase/firebaseui-core` package.

## Overview

The `@invertase/firebaseui-core` package is the framework-agnostic core of the Firebase UI for Web library. It provides a set of functions and utilities for building UIs with Firebase Authentication. The core package is designed to be used by framework-specific packages like `@invertase/firebaseui-react` and `@invertase/firebaseui-angular`, but it can also be used directly to build custom UIs.
...
```typescript
import { initializeUI, signInWithEmailAndPassword } from "@invertase/firebaseui-core";
...
```typescript
import { initializeUI, requireDisplayName } from "@invertase/firebaseui-core";
