...
if [[ $1 = "--saucelabs" ]]; then
  ...
  echo "$PROTRACTOR_BIN_PATH/webdriver-manager update --versions.chrome $(google-chrome --version)  --gecko=false"
  $PROTRACTOR_BIN_PATH/webdriver-manager update --versions.chrome $(google-chrome --version) --gecko=false
